...
| T-33 | Credential stuffing or brute-force against a password account | Supabase rate limits on sign-in attempts; non-enumerating responses; account lockout and suspicious-sign-in alerting | Password-specific abuse monitoring; password-entry throttling; never return "wrong password" vs "no such user" distinctions |
| T-34 | Account enumeration via sign-up, sign-in, or password-reset responses | Uniform client responses; confirmation-code sent only after a valid sign-up/update request; no existence hints in error copy or timing | Test every password flow with existing and non-existing emails and confirm identical visible behavior |
| T-35 | Very long password silently truncated by bcrypt's 72-byte cap | Client enforces 8–72 UTF-8 bytes before forwarding; password manager users warned only when their generated secret exceeds the supported length | Byte-length boundary tests; do not rely on JavaScript string length |
| T-36 | Password-manager desync from credential mutation without recent reauthentication | App-level proof prompts for link/unlink/email-change/password-change; redacted audit/security notices; future hardening to require reauthentication for all credential mutations | Treat password changes like identity mutations in the app flow until a server-side reauth gate is available |
