...
| Quiet schedules | Account quiet hours and per-source schedules suppress eligible pushes after the inbox record is stored. Their saved window follows the phone's current time zone automatically. | Quiet hours do not reject or delete alerts. Open and save a schedule after changing the phone's time zone so the server receives the new zone. |
...
| Severity | Optional `low`, `medium`, `high`, and `critical` values change the notification/inbox presentation. Severity card tints follow the active theme's light or dark appearance so text stays readable in both. | Severity is presentation metadata, not delivery priority. Omitted severity stays neutral. |
...
| Themes | Meadow, Ocean, Sunset, Violet, Minimalist, Marshmallow, and Neon recolor the app, system chrome, neutral alert cards, and iOS Live Status; the choice is stored on the device. Audited text/surface pairings are locked by an automated WCAG contrast test. | Theme choice is presentation only and does not change severity meaning or authorization. |
